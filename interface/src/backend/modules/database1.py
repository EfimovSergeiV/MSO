...
class RequestsDB():
    ...
    def create_cursor(self, sql_request):   #   create_requests
        """ Создание запросов к базе данных """
        cursor_object = self.cursor.execute(sql_request)
        return cursor_object

    def commit_cursor(self):
        """ Коммитит запросы к базе данных и закрывает соединение """

        self.conn.commit()
        self.conn.close()


def create_programm(id=None, data=None):
    """ Создание программы сварки """

    db = RequestsDB()
    foreign_keys = {}

    weldingProgrammData = data
    
    programm = weldingProgrammData.pop("programm_programmmodel")
    
    #CRUNCH DATATIME
    programm['created_at'] = str(dt_now)
    programm['updated_at'] = str(dt_now)
...
def create_or_update(data):
    ...
    db = RequestsDB()
    db.commit_cursor()
    programm = data['programm_programmmodel']
    id = 541

    if programm['id'] == 0:
        #   Create programm
        #   CRUNCH DATATIME AND PROGRAMM ID
        del programm['id']
        programm['created_at'] = str(dt_now)
        programm['updated_at'] = str(dt_now)
        ...
        programm = db.create_cursor(request)
        # foreign_keys['programm_programmmodel'] = {'programm_id': programm.lastrowid}
        id = programm.lastrowid

    else:
        # Update programm
        programm['updated_at'] = str(dt_now)
        print(f'UPDATE: {programm}')
